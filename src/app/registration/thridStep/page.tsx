...
export default function ThirdStep() {
  ...
  useEffect(() => {
    const loadUserId = async () => {
      try {
        const currentUserId = await getCurrentUserForRedirect();
        setUserId(currentUserId);
      } catch (error) {
        console.error('Error loading user ID:', error);
        // Fallback to URL param if server action fails
    ...
    return (
      <div className='relative box-border flex size-full min-h-screen flex-col content-stretch items-center justify-center bg-[#f7f9f8] px-[24px]'>
        <div className='relative flex flex-col items-center gap-[24px]'>
          <div className='relative size-[58px] shrink-0 animate-pulse'>
          ...
          <div className='flex items-center gap-[8px]'>
            <div className='h-[8px] w-[8px] animate-bounce rounded-full bg-[#2a7f62] [animation-delay:-0.3s]'></div>
            <div className='h-[8px] w-[8px] animate-bounce rounded-full bg-[#2a7f62] [animation-delay:-0.15s]'></div>
            <div className='h-[8px] w-[8px] animate-bounce rounded-full bg-[#2a7f62]'></div>
          </div>
        </div>
      </div>
    );
    ...
    <div className='relative box-border flex size-full min-h-screen flex-col content-stretch items-center justify-between bg-[#f7f9f8] px-[24px] pt-[56px] pb-[24px]'>
      ...
      <div className='relative w-full max-w-md shrink-0'>
        ...
        <div className='relative box-border flex w-full flex-col content-stretch items-start gap-[12px] border-0 border-solid border-[transparent] bg-clip-padding'>
          <Link
            href={userId ? `/${userId}/dashboard` : '/dashboard'}
